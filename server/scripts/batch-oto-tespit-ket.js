/**
 * Tüm KET projeleri için Yeni Durum Proje DXF üzerinden otomatik tespit
 * → proje_kesif_metraj tablosuna direk-bazlı satırlar yazar.
 *
 * Server zaten ayakta olmalı (DXF parse için /api/dosya/:id/dxf-elemanlar çağırıyor).
 * Kullanım:
 *   node server/scripts/batch-oto-tespit-ket.js [--apply] [--proje-id=N]
 */
...
// Hak Ediş'teki TIP_TUR_MAP ile aynı (frontend kopyası)
const TIP_TUR_MAP = {
...
function hesaplaOtoMalzemeler(tip, yakinlar) {
  ...
  // HSTA — direk tipi (S) ile bitiyor: G-N-14(S), N-14(S) vb. → HSTA grubu
...
async function dxfElemanlariGetir(dosyaId) {
  const r = await fetch(`${SERVER_BASE}/api/dosya/${dosyaId}/dxf-elemanlar`);
  if (!r.ok) throw new Error(`DXF parse failed: ${r.status}`);
...
async function main() {
  ...
  // Hedef projeler: KET, ve yeni_durum_proje DXF'i olan
  let where = "WHERE it.kod = 'KET'";
  ...
  console.log(`KET proje: ${projeler.length} | Yeni Durum DXF olan: ${hedefler.length}`);

  if (!APPLY) {
    console.log('\n[DRY-RUN] DXF\'i olan projeler:');
    for (const p of hedefler) console.log(`  ${p.proje_no.padEnd(20)} ${p.musteri_adi}  (dxf_id=${p.dxf_id})`);
  ...
  `);
  const sayMevcutStmt = db.prepare(`SELECT id, nokta1 FROM proje_kesif_metraj WHERE proje_id = ?`);
  ...
  for (const p of hedefler) {
    try {
      ...
      if (!elemanlar.length) {
        console.log(`  ⚠ ${p.proje_no}: DXF parse boş`);
        continue;
      }
      const anaDirekler = elemanlar.filter(d => d.numara && d.sembol && SEMBOL_DURUM[d.sembol]);
      if (!anaDirekler.length) {
        console.log(`  ⚠ ${p.proje_no}: ana direk bulunamadı`);
        continue;
      }

      // Önce mevcut satırları sil (idempotent)
      ...
      const tx = db.transaction(() => {
        ...
        for (const d of anaDirekler) {
          ...
          // Müşterek hat: text "3xSW + 4P+R" → AG ve OG tarafları ayrı parçala
          ...
          // Notlar formatı: tip parantezsiz, durum 5. parça
          ...
          insertStmt.run(
            p.id, sira,
            d.numara, komsu?.numara || null, direkDurum,
