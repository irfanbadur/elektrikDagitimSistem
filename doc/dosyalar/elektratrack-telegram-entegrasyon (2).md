...
**Frontend ek paketleri:**

| Paket | Amaç |
|-------|------|
| `leaflet` | Harita motoru (ücretsiz, API key gereksiz) |
| `react-leaflet` | Leaflet'in React wrapper'ı |
| `@turf/turf` | Coğrafi hesaplamalar (mesafe, alan, buffer) |
...
-- ============================================
-- SAHA HARİTA — PROJE GÜZERGAH / NOKTALARI
-- Projenin harita üzerindeki çizimi
-- Direk noktaları, kablo güzergahı, trafo konumları
-- ============================================
CREATE TABLE IF NOT EXISTS proje_harita_noktalari (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    proje_id INTEGER NOT NULL,
    nokta_tipi TEXT NOT NULL,
    -- 'direk'          → Direk konumu (nokta)
    -- 'trafo'          → Trafo konumu (nokta)
    -- 'pano'           → Pano/dolap konumu (nokta)
    -- 'baglanti_noktasi' → Hat başlangıç/bitiş noktası
    -- 'guzergah_noktasi' → Kablo güzergahındaki ara nokta
    -- 'isleme_noktasi'   → Özel işlem gereken nokta (geçit, traversal vb.)

    sira_no INTEGER,                         -- Güzergah üzerindeki sıra numarası
    latitude REAL NOT NULL,
    longitude REAL NOT NULL,
    etiket TEXT,                              -- 'D-1', 'D-2', 'TR-1', 'P-1' gibi kısa etiket
    aciklama TEXT,                            -- 'Köşe direği', 'Gergi direği', 'Trafo binası'
    durum TEXT DEFAULT 'planli',
    -- 'planli'     → Projede var, henüz yapılmadı
    -- 'devam'      → Yapım aşamasında
    -- 'tamamlandi' → Tamamlandı
    -- 'iptal'      → İptal edildi

    ikon TEXT,                               -- Haritadaki ikon tipi (frontend için)
    -- 'pole_planned', 'pole_progress', 'pole_done'
    -- 'transformer', 'panel', 'junction'

    -- İlişkiler
    veri_paketi_id INTEGER,                  -- Tamamlanma fotoğrafı olan veri paketi
    ekipman_bilgisi TEXT,                    -- JSON: bu noktadaki ekipmanlar
...
-- ============================================
-- SAHA HARİTA — GÜZERGAH ÇİZGİLERİ
-- İki nokta arası kablo hattı / boru hattı çizgileri
-- ============================================
CREATE TABLE IF NOT EXISTS proje_harita_cizgiler (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    proje_id INTEGER NOT NULL,
    baslangic_nokta_id INTEGER NOT NULL,     -- proje_harita_noktalari.id
    bitis_nokta_id INTEGER NOT NULL,         -- proje_harita_noktalari.id
    cizgi_tipi TEXT DEFAULT 'kablo',
    -- 'kablo_og'    → OG havai hat
    -- 'kablo_ag'    → AG hat
    -- 'kablo_yeralti' → Yeraltı kablo
    -- 'fiber'       → Fiber optik
    -- 'boru'        → Boru hattı

    durum TEXT DEFAULT 'planli',             -- 'planli', 'devam', 'tamamlandi'
    renk TEXT,                               -- Haritadaki çizgi rengi (#FF0000)
    kalinlik INTEGER DEFAULT 3,              -- Çizgi kalınlığı (px)
    kesikli BOOLEAN DEFAULT 0,              -- Kesikli çizgi mi? (planlı hatlar için)
    mesafe_m REAL,                           -- İki nokta arası mesafe (metre)
    aciklama TEXT,
    iletken_tipi TEXT,                       -- 'ACSR 95mm²', 'XLPE 3x150' vb.

    FOREIGN KEY (proje_id) REFERENCES projeler(id),
    FOREIGN KEY (baslangic_nokta_id) REFERENCES proje_harita_noktalari(id),
    FOREIGN KEY (bitis_nokta_id) REFERENCES proje_harita_noktalari(id)
);

-- ============================================
-- SAHA HARİTA — EKİP KONUM GEÇMİŞİ
-- Ekiplerin sahadaki konum geçmişi
-- Telegram'dan gelen konum verileri
-- ============================================
CREATE TABLE IF NOT EXISTS ekip_konum_gecmisi (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    ekip_id INTEGER NOT NULL,
    personel_id INTEGER,                     -- Konumu gönderen personel
    latitude REAL NOT NULL,
    longitude REAL NOT NULL,
    konum_kaynagi TEXT DEFAULT 'telegram',   -- 'telegram', 'web', 'otomatik'
    tarih DATETIME DEFAULT CURRENT_TIMESTAMP,
...
-- ============================================
-- SAHA HARİTA — ALAN/BÖLGE SINIRLARI
-- Çalışma bölgesi sınırları (polygon)
-- ============================================
CREATE TABLE IF NOT EXISTS proje_harita_alanlar (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    proje_id INTEGER,
    bolge_id INTEGER,
    alan_tipi TEXT,
    -- 'calisma_alani'   → Çalışma sahası sınırı
    -- 'enerji_kesinti'  → Enerji kesilecek bölge
    -- 'guvenlik'        → Güvenlik bölgesi
    -- 'izin_alani'      → Belediye izin alanı

    alan_adi TEXT,
    koordinatlar TEXT NOT NULL,              -- JSON: [[lat,lng],[lat,lng],...] polygon noktaları
    renk TEXT DEFAULT '#3388ff',
    opaklık REAL DEFAULT 0.2,
    aciklama TEXT,
    aktif BOOLEAN DEFAULT 1,
...
-- ============================================
-- INDEXLER (Saha Harita tabloları)
-- ============================================
CREATE INDEX IF NOT EXISTS idx_harita_nokta_proje ON proje_harita_noktalari(proje_id);
CREATE INDEX IF NOT EXISTS idx_harita_nokta_durum ON proje_harita_noktalari(durum);
CREATE INDEX IF NOT EXISTS idx_harita_nokta_konum ON proje_harita_noktalari(latitude, longitude);
CREATE INDEX IF NOT EXISTS idx_harita_cizgi_proje ON proje_harita_cizgiler(proje_id);
CREATE INDEX IF NOT EXISTS idx_ekip_konum_ekip ON ekip_konum_gecmisi(ekip_id);
CREATE INDEX IF NOT EXISTS idx_ekip_konum_tarih ON ekip_konum_gecmisi(tarih);
...
```
│  📊 Dashboard       │
│  🗺️ Saha            │  ← YENİ (harita üzerinde canlı saha görünümü)
│  👥 Ekipler         │
│  🏗️ Projeler        │
│  📦 Malzeme         │
│  👤 Personel        │
...
│  ✅ Görevler        │
│  📸 Veri Paketleri  │
│  🗺️ Saha            │  ← YENİ (canlı harita, ekip konum, ilerleme)
│  🔧 Ekipman Katalog │
│  📈 Raporlar        │
│  ──────────────     │
│  🤖 AI & Telegram   │
...
## 🗺️ Saha Modülü — Canlı Harita Görünümü

### Teknoloji

| Paket | Amaç | Maliyet |
|-------|------|---------|
| `react-leaflet` | React Leaflet wrapper | Ücretsiz |
| `leaflet` | Harita motoru | Ücretsiz |
| `leaflet-draw` | Harita üzerine çizim (hat güzergahı) | Ücretsiz |
| `leaflet.markercluster` | Çok sayıda marker gruplama | Ücretsiz |
| **OpenStreetMap Tiles** | Harita karoları (tile) | **Ücretsiz, API key yok, sınırsız** |

```bash
cd client
npm install react-leaflet leaflet leaflet-draw leaflet.markercluster
```

> **Neden Leaflet + OSM?**  
> API key gerektirmez, kullanım limiti yoktur, ticari ürünlerde ücretsizdir.
> Sadece tile attributionı göstermek zorunlu: `© OpenStreetMap contributors`.
> İnternet olmadan da çalışabilir (offline tile pack ile).

### Harita Katmanları (Layers)

Saha haritası 7 katmandan oluşur. Kullanıcı her katmanı açıp kapatabilir:

```
┌─────────────────────────────────────────────────────┐
│  Katman Kontrolü (sağ üst)         [Tümünü Aç/Kapa]│
│                                                     │
│  ☑ Ekipler          → Ekiplerin anlık konumu        │
│  ☑ Projeler         → Proje konumları + durum rengi │
│  ☐ Hat Güzergahları → Kablo/hat çizgileri           │
│  ☐ Veri Paketleri   → Saha fotoğraf konumları       │
│  ☐ Malzeme Noktaları→ Depo, malzeme kullanım yeri   │
│  ☐ Hasar/Arıza      → Hasar tespit noktaları        │
│  ☐ Isı Haritası     → İş yoğunluğu heat map         │
└─────────────────────────────────────────────────────┘
```

### Veritabanı Eklentileri

Mevcut tablolara koordinat alanları eklenir + yeni tablo:

```sql
-- ============================================
-- MEVCUT TABLOLARA KOORDİNAT EKLEMELERİ
-- ============================================

-- Projeler tablosuna (zaten bolge_id var, koordinat eklenir)
ALTER TABLE projeler ADD COLUMN latitude REAL;          -- Proje merkez noktası
ALTER TABLE projeler ADD COLUMN longitude REAL;
ALTER TABLE projeler ADD COLUMN adres TEXT;              -- Açık adres

-- Ekipler tablosuna (son bilinen konum)
ALTER TABLE ekipler ADD COLUMN son_latitude REAL;       -- Ekibin son bilinen konumu
ALTER TABLE ekipler ADD COLUMN son_longitude REAL;
ALTER TABLE ekipler ADD COLUMN son_konum_zamani DATETIME;
ALTER TABLE ekipler ADD COLUMN son_konum_kaynagi TEXT;  -- 'telegram', 'manuel', 'veri_paketi'

-- ============================================
-- HAT GÜZERGAHLARİ
-- Projelerin kablo/hat çizimleri (polyline)
-- ============================================
CREATE TABLE IF NOT EXISTS hat_guzergahlari (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    proje_id INTEGER NOT NULL,
    guzergah_adi TEXT,                       -- 'Ana hat', 'Branşman 1'
    guzergah_tipi TEXT NOT NULL,
    -- 'og_havai'        → OG havai hat
    -- 'og_yeralti'      → OG yeraltı kablo
    -- 'ag_havai'         → AG havai hat  
    -- 'ag_yeralti'       → AG yeraltı kablo
    -- 'dagitim'          → Dağıtım hattı
    -- 'aydinlatma'       → Aydınlatma hattı

    koordinatlar TEXT NOT NULL,              -- JSON: [[lat,lng], [lat,lng], ...]
    -- GeoJSON LineString formatı
    -- Örnek: [[41.6833,35.9167],[41.6840,35.9170],[41.6850,35.9180]]

    toplam_uzunluk_m REAL,                   -- Metre cinsinden hat uzunluğu
    iletken_tipi TEXT,                       -- 'ACSR 95mm²', 'XLPE 3x150' vb.
    direk_sayisi INTEGER,                    -- Hat üzerindeki direk sayısı

    durum TEXT DEFAULT 'planlandi',
    -- 'planlandi'       → Henüz yapılmadı (kesikli çizgi)
    -- 'devam_ediyor'    → Yapım aşamasında (turuncu)
    -- 'tamamlandi'      → Tamamlandı (yeşil)
    -- 'iptal'           → İptal (gri)

    tamamlanma_yuzdesi INTEGER DEFAULT 0,    -- 0-100
    renk TEXT,                               -- Haritadaki çizgi rengi (override)
    notlar TEXT,
...
-- ============================================
-- DIREK NOKTALARI
-- Hat üzerindeki her direğin konumu
-- ============================================
CREATE TABLE IF NOT EXISTS direk_noktalari (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    hat_guzergah_id INTEGER,                 -- Hangi hatta
    proje_id INTEGER NOT NULL,
    direk_no INTEGER,                        -- Hat üzerindeki sıra no: 1, 2, 3...
    direk_adi TEXT,                           -- 'D-1', 'D-2' veya özel isim

    latitude REAL NOT NULL,
    longitude REAL NOT NULL,

    direk_tipi TEXT,                          -- 'beton', 'ahsap', 'celik'
    direk_boy_m REAL,                        -- Boy (metre)
    gerilim_sinifi TEXT,                     -- 'AG', 'OG'
    direk_fonksiyon TEXT,                    -- 'kose', 'gergi', 'tasiyi', 'son', 'dagitim'

    durum TEXT DEFAULT 'planlandi',
    -- 'planlandi'       → Proje aşamasında
    -- 'dikildi'         → Direk dikildi
    -- 'donatildi'       → Konsol/izolatör takıldı
    -- 'iletken_cekildi' → İletken çekildi
    -- 'tamamlandi'      → Tamamen bitmiş
    -- 'hasarli'         → Hasar var

    montaj_tarihi DATE,
    son_kontrol_tarihi DATE,
    veri_paketi_id INTEGER,                  -- İlişkili saha veri paketi
    foto_analiz_id INTEGER,                  -- AI analiz sonucu

    notlar TEXT,
    olusturma_tarihi DATETIME DEFAULT CURRENT_TIMESTAMP,

    FOREIGN KEY (hat_guzergah_id) REFERENCES hat_guzergahlari(id),
    FOREIGN KEY (proje_id) REFERENCES projeler(id),
    FOREIGN KEY (veri_paketi_id) REFERENCES veri_paketleri(id),
    FOREIGN KEY (foto_analiz_id) REFERENCES foto_analiz(id)
);
...
-- ============================================
-- SAHA OLAYLARI (Genel amaçlı konum işaretçisi)
-- Hasar, arıza, kesinti, özel durum noktaları
-- ============================================
CREATE TABLE IF NOT EXISTS saha_olaylari (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    olay_tipi TEXT NOT NULL,
    -- 'hasar'           → Ekipman hasarı
    -- 'ariza'           → Arıza bildirimi
    -- 'kesinti'         → Enerji kesintisi
    -- 'tehlike'         → Güvenlik tehlikesi
    -- 'not'             → Genel saha notu
    -- 'depo'            → Malzeme depo/stok noktası
    -- 'toplanti'        → Toplantı/buluşma noktası
    ...
    proje_id INTEGER,
    ekip_id INTEGER,
    bildirim_yapan_id INTEGER,

    durum TEXT DEFAULT 'aktif',              -- 'aktif', 'cozuldu', 'iptal'
    ...
    FOREIGN KEY (proje_id) REFERENCES projeler(id),
    FOREIGN KEY (ekip_id) REFERENCES ekipler(id),
    FOREIGN KEY (bildirim_yapan_id) REFERENCES personel(id)
);

-- INDEXLER
CREATE INDEX IF NOT EXISTS idx_hat_guzergah_proje ON hat_guzergahlari(proje_id);
CREATE INDEX IF NOT EXISTS idx_direk_noktalari_proje ON direk_noktalari(proje_id);
CREATE INDEX IF NOT EXISTS idx_direk_noktalari_hat ON direk_noktalari(hat_guzergah_id);
CREATE INDEX IF NOT EXISTS idx_direk_noktalari_konum ON direk_noktalari(latitude, longitude);
CREATE INDEX IF NOT EXISTS idx_saha_olaylari_konum ON saha_olaylari(latitude, longitude);
CREATE INDEX IF NOT EXISTS idx_saha_olaylari_proje ON saha_olaylari(proje_id);
```

### API Endpoint'leri

```
# ─── SAHA HARİTA VERİLERİ ────────────────────────────────
# Tek istekle haritadaki tüm katmanları çek (performans için)
GET    /api/saha/genel-gorunum           → Tüm katmanlar özet (bbox filtresi ile)
       ?bbox=41.5,35.8,41.8,36.1         → Harita görünüm alanı (bounding box)
       &katmanlar=ekipler,projeler       → Sadece istenen katmanlar
       &proje_id=5                       → Tek projeye odaklan

# ─── EKİP KONUMLARI ──────────────────────────────────────
GET    /api/saha/ekipler                 → Tüm ekiplerin son konumları
PUT    /api/saha/ekipler/:id/konum       → Ekip konumunu güncelle {lat, lng, kaynak}

# ─── PROJE KONUMLARI ─────────────────────────────────────
GET    /api/saha/projeler                → Tüm projelerin konumları + durum özeti
PUT    /api/saha/projeler/:id/konum      → Proje koordinatı güncelle

# ─── HAT GÜZERGAHLARİ ────────────────────────────────────
GET    /api/saha/guzergahlar             → Tüm güzergahlar (filtre: ?proje_id=)
GET    /api/saha/guzergahlar/:id         → Güzergah detay (direk noktaları dahil)
POST   /api/saha/guzergahlar             → Yeni güzergah oluştur (haritadan çizim)
PUT    /api/saha/guzergahlar/:id         → Güzergah güncelle (koordinatlar, durum)
DELETE /api/saha/guzergahlar/:id         → Güzergah sil

# ─── DİREK NOKTALARI ─────────────────────────────────────
GET    /api/saha/direkler                → Tüm direkler (filtre: ?proje_id=&hat_id=&durum=)
GET    /api/saha/direkler/:id            → Direk detay (analiz, fotoğraflar dahil)
POST   /api/saha/direkler                → Yeni direk noktası ekle (haritadan)
PUT    /api/saha/direkler/:id            → Direk güncelle (durum, tip, notlar)
PUT    /api/saha/direkler/:id/durum      → Direk durumunu güncelle (ilerleme takibi)
DELETE /api/saha/direkler/:id            → Direk sil

# ─── SAHA OLAYLARI ────────────────────────────────────────
GET    /api/saha/olaylar                 → Aktif saha olayları (hasar, arıza, kesinti)
POST   /api/saha/olaylar                 → Yeni olay bildir (haritadan veya Telegram'dan)
PUT    /api/saha/olaylar/:id             → Olay güncelle / çözüldü işaretle
DELETE /api/saha/olaylar/:id             → Olay sil

# ─── İSTATİSTİK / ÖZET ───────────────────────────────────
GET    /api/saha/istatistik              → Bölge bazlı ilerleme özeti
GET    /api/saha/isi-haritasi            → Heat map verisi (iş yoğunluğu)
```

### `GET /api/saha/genel-gorunum` Yanıt Formatı

```javascript
// Bu endpoint haritanın ilk yüklenişinde çağrılır
// Tüm aktif verileri tek seferde döndürür
{
  "merkez": { "lat": 41.2867, "lng": 36.3300 },  // Firma il merkezi (Samsun)
  "zoom": 10,

  "ekipler": [
    {
      "id": 1, "ekip_adi": "Ekip 1", "ekip_kodu": "EK-01",
      "lat": 41.6833, "lng": 35.9167,
      "son_konum_zamani": "2026-02-08T14:30:00",
      "aktif_proje": "YB-2025-001",
      "personel_sayisi": 4,
      "bugunki_rapor": true   // Günlük rapor gönderdi mi?
    }
  ],

  "projeler": [
    {
      "id": 5, "proje_no": "YB-2025-001", "proje_adi": "Bafra YB",
      "lat": 41.5667, "lng": 35.9000,
      "proje_tipi": "YB", "durum": "devam_ediyor",
      "ilerleme_yuzdesi": 65,
      "ekip": "Ekip 1",
      "baslama_tarihi": "2025-11-01",
      "bitis_tarihi": "2026-03-15"
    }
  ],

  "guzergahlar": [
    {
      "id": 1, "proje_id": 5, "guzergah_adi": "Ana hat",
      "tip": "og_havai", "durum": "devam_ediyor",
      "tamamlanma_yuzdesi": 70,
      "koordinatlar": [[41.5667,35.9000],[41.5680,35.9020],[41.5700,35.9050]],
      "direk_sayisi": 15, "uzunluk_m": 2400,
      "iletken": "ACSR 95mm²"
    }
  ],
  ...
  "direkler": [
    {
      "id": 1, "proje_id": 5, "hat_id": 1,
      "direk_no": 1, "direk_adi": "D-1",
      "lat": 41.5667, "lng": 35.9000,
      "tip": "beton", "boy": 12, "fonksiyon": "kose",
      "durum": "tamamlandi",
      "foto_sayisi": 3, "analiz_var": true
    },
    {
      "id": 2, "direk_no": 2,
      "lat": 41.5680, "lng": 35.9020,
      "durum": "iletken_cekildi"
      // ...
    }
  ],
  ...
  "olaylar": [
    {
      "id": 1, "tip": "hasar", "oncelik": "yuksek",
      "lat": 41.5700, "lng": 35.9050,
      "baslik": "3 nolu direk kırık izolatör",
      "tarih": "2026-02-08T09:00:00",
      "durum": "aktif"
    }
  ],
  ...
  "veri_paketleri": [
    {
      "id": 42, "paket_no": "VP-2025-0042",
      "lat": 41.6833, "lng": 35.9167,
      "tip": "direk_tespit", "foto_sayisi": 3,
      "tarih": "2026-02-08T09:15:00"
    }
  ]
}
```

### Frontend — Saha Sayfası Ana Ekran Wireframe

```
┌──────────────────────────────────────────────────────────────────────────┐
│  🗺️ Saha Görünümü          [Proje: Tümü ▾] [Tarih: Bugün ▾] [🔄 Yenile]│
├──────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│  ┌─────────────────────────────────────────────────────┐  ┌──────────┐  │
│  │                                                     │  │ KATMANLAR│  │
...
│  │                                                     │  │ ☑ Ekipler│  │
│  │         🔵 Ekip 1                                   │  │ ☑ Proje  │  │
│  │           (4 kişi, YB-001)                          │  │ ☐ Hatlar │  │
│  │                                                     │  │ ☐ Direk  │  │
│  │    🟢──────🟢──────🟡──────⚪──────⚪               │  │ ☐ Foto   │  │
│  │    D-1     D-2     D-3     D-4     D-5              │  │ ☐ Olay   │  │
│  │    ✅       ✅      🔨      📋      📋              │  │ ☐ Isı    │  │
│  │                                                     │  │          │  │
│  │              🔴 Hasar: Kırık izolatör               │  ├──────────┤  │
│  │                                                     │  │ ÖZET     │  │
│  │         📸 VP-0042                                  │  │          │  │
│  │           (3 foto, direk tespit)                    │  │ Proje: 9 │  │
│  │                                                     │  │ Ekip:  3 │  │
│  │    🟢 Tamamlandı                                    │  │ Direk:42 │  │
│  │    🟡 Devam Ediyor                                  │  │ Hat: 12km│  │
│  │    ⚪ Planlandı           🔵 Ekip 2                 │  │ Olay:  3 │  │
│  │    🔴 Hasar/Arıza          (3 kişi, KET-011)       │  │          │  │
│  │                                                     │  │ İlerleme │  │
│  │    ┌──────────────── © OSM ─────────────────────┐  │  │ ████░ 65%│  │
│  └─────────────────────────────────────────────────────┘  └──────────┘  │
│                                                                          │
│  ┌──────────────────────────────────────────────────────────────────────┐│
│  │  Alt Panel (marker'a tıklayınca açılır)                              ││
│  │                                                                      ││
│  │  📍 D-3 | Direk #3 | YB-2025-001 Bafra                             ││
│  │  Tip: Beton 12m | Durum: 🔨 Donatılıyor | Ekip 1                   ││
│  │  Son güncelleme: 08.02.2026 14:30                                   ││
│  │                                                                      ││
│  │  [📸 Fotoğraflar(2)] [🤖 AI Analiz] [📝 Not Ekle] [✏️ Düzenle]    ││
│  └──────────────────────────────────────────────────────────────────────┘│
│                                                                          │
└──────────────────────────────────────────────────────────────────────────┘
```

### Harita Renk Kodları ve İkonlar

```javascript
// Marker ve çizgi renk/ikon tanımları

const RENK_KODLARI = {
  // Proje durumları
  proje: {
    planlandi:     '#9CA3AF',  // Gri
    basladi:       '#3B82F6',  // Mavi
    devam_ediyor:  '#F59E0B',  // Turuncu
    tamamlandi:    '#10B981',  // Yeşil
    askiya_alindi: '#EF4444',  // Kırmızı
  },

  // Direk durumları
  direk: {
    planlandi:        '#D1D5DB',  // Açık gri (boş daire)
    dikildi:          '#93C5FD',  // Açık mavi
    donatildi:        '#FCD34D',  // Sarı
    iletken_cekildi:  '#6EE7B7',  // Açık yeşil
    tamamlandi:       '#10B981',  // Yeşil (dolu daire)
    hasarli:          '#EF4444',  // Kırmızı
  },

  // Hat güzergahı
  hat: {
    og_havai:   '#DC2626',  // Kırmızı (OG)
    og_yeralti: '#991B1B',  // Koyu kırmızı (OG yeraltı, kesikli)
    ag_havai:   '#2563EB',  // Mavi (AG)
    ag_yeralti: '#1E40AF',  // Koyu mavi (AG yeraltı, kesikli)
    aydinlatma: '#F59E0B',  // Sarı
  },

  // Hat durumu (çizgi stili)
  hat_durum: {
    planlandi:     { dashArray: '10, 10', opacity: 0.4 },  // Kesikli, soluk
    devam_ediyor:  { dashArray: '10, 5',  opacity: 0.8 },  // Kesikli, belirgin
    tamamlandi:    { dashArray: null,      opacity: 1.0 },  // Düz çizgi
  },

  // Olay tipleri
  olay: {
    hasar:    '#EF4444',  // Kırmızı
    ariza:    '#F97316',  // Turuncu
    kesinti:  '#8B5CF6',  // Mor
    tehlike:  '#DC2626',  // Koyu kırmızı
    not:      '#6B7280',  // Gri
    depo:     '#059669',  // Yeşil
  }
};

// Ekip marker ikonu (renkli daire + ekip kodu)
const ekipIcon = (ekip) => L.divIcon({
...
// Direk marker ikonu (durum rengine göre)
const direkIcon = (direk) => L.divIcon({
  className: 'direk-marker',
  html: `<div style="
    background: ${RENK_KODLARI.direk[direk.durum]};
...
### Frontend — React Bileşen Yapısı

```
client/src/pages/
├── SahaPage.jsx                    # Ana saha sayfası (harita + paneller)
├── saha/
│   ├── SahaHarita.jsx              # Leaflet harita bileşeni
│   ├── katmanlar/
│   │   ├── EkipKatmani.jsx         # Ekip marker'ları
│   │   ├── ProjeKatmani.jsx        # Proje marker'ları
│   │   ├── HatGuzergahKatmani.jsx  # Polyline çizgileri
│   │   ├── DirekKatmani.jsx        # Direk noktaları
│   │   ├── VeriPaketiKatmani.jsx   # Fotoğraf konumları
│   │   ├── OlayKatmani.jsx         # Hasar/arıza marker'ları
│   │   └── IsiHaritasiKatmani.jsx  # Heat map
│   ├── paneller/
│   │   ├── KatmanKontrol.jsx       # Katman açma/kapama
│   │   ├── OzetPanel.jsx           # Sağ taraf özet bilgiler
│   │   ├── DetayPanel.jsx          # Alt panel (marker detayı)
│   │   ├── ProjeDetayPanel.jsx     # Proje seçildiğinde
│   │   ├── DirekDetayPanel.jsx     # Direk seçildiğinde
│   │   └── GuzergahCizim.jsx       # Hat güzergahı çizim aracı
│   └── hooks/
│       ├── useSahaVerileri.js       # API'den saha verisi çek
│       ├── useHaritaKonumu.js       # Harita merkez/zoom yönetimi
│       └── useOtoYenileme.js        # Otomatik yenileme (30sn)
```

### `SahaHarita.jsx` — Ana Harita Bileşeni
...
import EkipKatmani from './katmanlar/EkipKatmani';
import ProjeKatmani from './katmanlar/ProjeKatmani';
import HatGuzergahKatmani from './katmanlar/HatGuzergahKatmani';
import DirekKatmani from './katmanlar/DirekKatmani';
import VeriPaketiKatmani from './katmanlar/VeriPaketiKatmani';
import OlayKatmani from './katmanlar/OlayKatmani';
import DetayPanel from './paneller/DetayPanel';
import OzetPanel from './paneller/OzetPanel';

const { Overlay } = LayersControl;

export default function SahaHarita({ firmaAyarlari }) {
  const [sahaVerisi, setSahaVerisi] = useState(null);
  const [seciliNesne, setSeciliNesne] = useState(null);
  const [filtreler, setFiltreler] = useState({ proje_id: null });

  // Firma il merkezini varsayılan merkez olarak kullan
  const merkez = firmaAyarlari?.harita_merkez
    || [41.2867, 36.3300]; // Samsun varsayılan

  // Veri yükleme (ilk açılış + otomatik yenileme)
  useEffect(() => {
    const yukle = async () => {
      const params = new URLSearchParams();
      if (filtreler.proje_id) params.set('proje_id', filtreler.proje_id);

      const res = await fetch(`/api/saha/genel-gorunum?${params}`);
      const data = await res.json();
      setSahaVerisi(data);
    };

    yukle();
    const interval = setInterval(yukle, 30000); // 30 saniyede bir yenile
    return () => clearInterval(interval);
  }, [filtreler]);

  if (!sahaVerisi) return <div>Harita yükleniyor...</div>;
    ...
    <div style={{ display: 'flex', height: 'calc(100vh - 64px)' }}>
      ...
      <div style={{ flex: 1, position: 'relative' }}>
        ...
        >
          ...
          {/* Katman kontrollü overlay'ler */}
          <LayersControl position="topright">

            <Overlay checked name="🔵 Ekipler">
              <EkipKatmani
                ekipler={sahaVerisi.ekipler}
                onSelect={(e) => setSeciliNesne({ tip: 'ekip', veri: e })}
              />
            </Overlay>

            <Overlay checked name="🏗️ Projeler">
              <ProjeKatmani
                projeler={sahaVerisi.projeler}
                onSelect={(p) => setSeciliNesne({ tip: 'proje', veri: p })}
              />
            </Overlay>

            <Overlay name="📏 Hat Güzergahları">
              <HatGuzergahKatmani
                guzergahlar={sahaVerisi.guzergahlar}
                onSelect={(g) => setSeciliNesne({ tip: 'guzergah', veri: g })}
              />
            </Overlay>

            <Overlay name="📍 Direkler">
              <DirekKatmani
                direkler={sahaVerisi.direkler}
                onSelect={(d) => setSeciliNesne({ tip: 'direk', veri: d })}
              />
            </Overlay>

            <Overlay name="📸 Veri Paketleri">
              <VeriPaketiKatmani
                paketler={sahaVerisi.veri_paketleri}
                onSelect={(vp) => setSeciliNesne({ tip: 'veri_paketi', veri: vp })}
              />
            </Overlay>

            <Overlay name="⚠️ Olaylar">
              <OlayKatmani
                olaylar={sahaVerisi.olaylar}
                onSelect={(o) => setSeciliNesne({ tip: 'olay', veri: o })}
              />
            </Overlay>
...
### Hat Güzergahı Çizim Özelliği

Koordinatör harita üzerinde hat güzergahı çizebilir ve direk noktaları işaretleyebilir:

```
┌──────────────────────────────────────────────────────────────────┐
│  🗺️ Güzergah Çizim Modu                          [İptal] [Kaydet]│
├──────────────────────────────────────────────────────────────────┤
│                                                                  │
│  Proje: [YB-2025-001 ▾]   Tip: [OG Havai Hat ▾]               │
│  İletken: [ACSR 95mm² ▾]                                       │
│                                                                  │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │                                                          │   │
│  │  Harita üzerine tıklayarak hat güzergahını çizin.       │   │
│  │  Her tıklama bir direk noktası ekler.                    │   │
│  │                                                          │   │
│  │     📍──────📍──────📍──────📍                           │   │
│  │     D-1     D-2     D-3     D-4                          │   │
│  │                                                          │   │
│  │  Toplam: 4 direk | 850m | 3 bölüm                       │   │
│  │                                                          │   │
│  │  [Geri Al] [Son Noktayı Sil] [Tamamla]                  │   │
│  └──────────────────────────────────────────────────────────┘   │
│                                                                  │
│  Direk Listesi:                                                  │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │ D-1 │ 41.5667, 35.9000 │ Köşe  │ Beton 12m │ [Düzenle] │   │
│  │ D-2 │ 41.5680, 35.9020 │ Taşıyı│ Beton 12m │ [Düzenle] │   │
│  │ D-3 │ 41.5700, 35.9040 │ Taşıyı│ Beton 10m │ [Düzenle] │   │
│  │ D-4 │ 41.5720, 35.9055 │ Son   │ Beton 12m │ [Düzenle] │   │
│  └──────────────────────────────────────────────────────────┘   │
│                                                                  │
└──────────────────────────────────────────────────────────────────┘
```

### Ekip Konum Güncelleme — Telegram Entegrasyonu

Ekiplerin haritadaki konumu otomatik güncellenir:

```javascript
// Telegram'dan konum geldiğinde (locationHandler.js'e eklenir)
async function updateTeamLocation(personelId, latitude, longitude) {
  const db = getDb();

  // Personelin ekibini bul
  const personel = db.prepare(
    'SELECT ekip_id FROM personel WHERE id = ?'
  ).get(personelId);
  ...
  if (personel?.ekip_id) {
    db.prepare(`
      UPDATE ekipler SET
        son_latitude = ?,
        son_longitude = ?,
        son_konum_zamani = CURRENT_TIMESTAMP,
        son_konum_kaynagi = 'telegram'
      WHERE id = ?
    `).run(latitude, longitude, personel.ekip_id);
  }
}

// Veri paketi oluşturulduğunda da konum güncellenir
// (dataBundleService.js'de paket konumu ekip konumunu da günceller)
```

### Firma Ayarları — Harita Varsayılanları

```sql
INSERT OR IGNORE INTO firma_ayarlari (anahtar, deger, aciklama) VALUES
('harita_merkez_lat', '41.2867', 'Harita varsayılan merkez enlemi'),
('harita_merkez_lng', '36.3300', 'Harita varsayılan merkez boylamı'),
('harita_varsayilan_zoom', '10', 'Harita varsayılan yakınlaştırma seviyesi'),
('harita_oto_yenileme_sn', '30', 'Harita otomatik yenileme süresi (saniye)');
```
...
### Faz 5 — Saha Harita Modülü
20. **Saha API'leri**: Genel görünüm, güzergah, direk, ekip konum endpoint'leri
21. **Leaflet harita**: SahaHarita.jsx, OpenStreetMap tile entegrasyonu
22. **Harita katmanları**: Ekip, proje, güzergah, direk, olay marker bileşenleri
23. **Güzergah çizim aracı**: Harita üzerinde hat çizme, direk noktası ekleme
24. **Detay panelleri**: Proje/direk/ekip seçildiğinde bilgi panelleri
25. **Otomatik yenileme**: 30sn'de bir ekip konumları güncelleme

### Faz 6 — Otomasyon + İyileştirme
26. **Cron görevleri**: Otomatik tamamlama, hatırlatmalar, günlük özetler
27. **Bildirim sistemi**: Koordinatöre acil durum bildirimleri
28. **Doğruluk takibi**: AI onay/düzeltme verilerinden doğruluk raporları
