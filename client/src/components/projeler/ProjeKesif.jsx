...
import { Plus, Trash2, Search, Package, Check, AlertCircle, Clock, X } from 'lucide-react'
...
function KesifFormSatiri({ onKaydet, onIptal }) {
  ...
  // Debounce: 300ms sonra arama yap
  useEffect(() => {
    if (secildi) return
  ...
  const handleAramaChange = (e) => {
    ...
    if (!val) {
      setForm({ malzeme_adi: '', malzeme_kodu: '', poz_no: '', birim: 'Ad', miktar: form.miktar, birim_fiyat: form.birim_fiyat })
    }
  }
                ...
                <table className="w-full text-xs">
                  <thead className="sticky top-0 bg-muted/90 backdrop-blur-sm">
                    <tr className="border-b border-input">
                      ...
                      <th className="px-2 py-1.5 text-left font-medium text-muted-foreground">SAP Tanimi</th>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-input bg-muted/50">
                ...
                <th className="px-3 py-3 text-right text-xs font-semibold text-muted-foreground w-24"></th>
              </tr>
            </thead>
            <tbody>
              ...
              {isLoading ? (
                ...
                kesifler?.map((k) => {
                  ...
                  return (
                    <KesifSatiri key={k.id} kalem={k} durum={durum} onGuncelle={(data) => guncelle.mutate({ id: k.id, ...k, ...data })} onDurumDegistir={(d) => handleDurumDegistir(k.id, k, d)} onSil={() => sil.mutate(k.id)} />
                  )
                })
              )}
            </tbody>
