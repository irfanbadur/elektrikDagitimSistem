...
export default function IrsaliyeGiris({ onBasarili }) {
  ...
  const [irsaliyeInfo, setIrsaliyeInfo] = useState({ bono_no: '', bono_tarihi: '', kurum: '', teslim_alan: '', aciklama: '' })
  ...
  const handleAnaliz = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/bonolar/parse-irsaliye', {
        method: 'POST',
      ...
      setSonuc(data)
      setIrsaliyeInfo({
        bono_no: data.irsaliye_no || '',
        bono_tarihi: data.irsaliye_tarihi || new Date().toISOString().split('T')[0],
        kurum: data.firma || '',
  ...
  const handleOnayla = async () => {
    ...
    if (!irsaliyeInfo.bono_no) {
      setHata('Irsaliye no zorunludur')
      return
    }
    if (!irsaliyeInfo.bono_tarihi) {
      setHata('Irsaliye tarihi zorunludur')
      return
    }
    ...
    try {
      ...
      setHata(err.message || 'Irsaliye kaydedilirken hata olustu')
    } finally {
  ...
  return (
    <div className="space-y-4">
      ...
      {!sonuc && (
        <div className="space-y-4">
          <p className="text-sm text-muted-foreground">
            Irsaliye gorselini yukleyin, AI analiz ederek malzemeleri otomatik cikaracaktir.
          </p>
          <div
          ...
          >
            ...
            {onizleme ? (
              <img src={onizleme} alt="Irsaliye onizleme" className="max-h-64 rounded-lg object-contain" />
            ) : (
              <>
                <Image className="mb-2 h-10 w-10 text-muted-foreground/40" />
                <p className="text-sm font-medium text-muted-foreground">Irsaliye gorselini yuklemek icin tiklayin</p>
      ...
      {/* Sonuc - Irsaliye bilgileri */}
      {sonuc && (
        <>
          <div className="rounded-lg border border-input bg-muted/30 p-4">
            <h4 className="mb-3 text-sm font-semibold">Irsaliye Bilgileri</h4>
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              <div>
                <label className="text-xs text-muted-foreground">Irsaliye No *</label>
                <input
                  value={irsaliyeInfo.bono_no}
                  onChange={e => setIrsaliyeInfo({ ...irsaliyeInfo, bono_no: e.target.value })}
              ...
              <div>
                ...
                <input
                  type="date"
                  value={irsaliyeInfo.bono_tarihi}
                  onChange={e => setIrsaliyeInfo({ ...irsaliyeInfo, bono_tarihi: e.target.value })}
                  className="mt-1 w-full rounded border border-input bg-background px-2 py-1.5 text-sm focus:border-primary focus:outline-none"
                />
              </div>
              <div>
                ...
                <input
                  value={irsaliyeInfo.kurum}
                  onChange={e => setIrsaliyeInfo({ ...irsaliyeInfo, kurum: e.target.value })}
                  className="mt-1 w-full rounded border border-input bg-background px-2 py-1.5 text-sm focus:border-primary focus:outline-none"
                />
              </div>
              <div>
            ...
            <div className="mt-3">
              ...
              <input
                value={irsaliyeInfo.aciklama}
                onChange={e => setIrsaliyeInfo({ ...irsaliyeInfo, aciklama: e.target.value })}
                className="mt-1 w-full rounded border border-input bg-background px-2 py-1.5 text-sm focus:border-primary focus:outline-none"
              />
            </div>
          </div>
